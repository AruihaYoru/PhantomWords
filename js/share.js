...
export function addShareButton(cardElement, word, definition, translatedDef) {
    ...
    button.addEventListener('click', async (e) => {
        ...
        if (navigator.share && navigator.canShare) {
            try {
                showFeedback(button, 'Generating image...');
                ...
                const file = new File([blob], `${word.toLowerCase()}.png`, { type: 'image/png' });
                ...
                if (navigator.canShare({ files: [file] })) {
                    ...
                    const feedback = button.querySelector('.share-feedback');
                    if (feedback) feedback.remove();
                } else {
                    await navigator.share({ title: `PhantomWord: ${word}`, text: `${shareText}\n\n${shareUrl}` });
                }

            } catch (err) {
                console.error('Error using Web Share API:', err);
                copyToClipboard(button, `${shareText}\n\n${shareUrl}`);
            }
        } else {
            copyToClipboard(button, `${shareText}\n\n${shareUrl}`);
        }
...
/**
 * テキストをクリップボードにコピーし、フィードバックを表示する
 * @param {HTMLElement} button - フィードバックを表示するボタン要素
...
function showFeedback(element, message) {
    ...
    if (message.includes('Generating')) return;
